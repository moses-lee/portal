...
export async function summarizeSession(meta: SessionMeta, project: Project | null): Promise<SessionSummary> {
  const { id, agentId, agentName, cwd, projectId, createdAt, lastActiveAt, title, busy, awaitingPermission, link, state } = meta;
  ...
  return {
    id, agentId, agentName, cwd, projectId, createdAt, lastActiveAt, title, busy, awaitingPermission, link, state,
