...
/** The sessions a scoped id names: the exact id, else every session it is a prefix of (old scopes hold 8-char prefixes). */
export function scopedSessions(sessions: SessionMeta[], id: string): SessionMeta[] {
  const exact = sessions.find((session) => session.id === id);
  return exact ? [exact] : sessions.filter((session) => session.id.startsWith(id));
}
...
function sessionRows(ids: string[], sessions: SessionMeta[] | null): string[] {
  ...
  return ids.map((id) => {
    const found = scopedSessions(sessions, id);
    ...
    const [session] = found;
    return `- ${session.id} "${session.title ?? "untitled"}" · ${session.agentName} · ${snapshotActivity(session)} · link ${session.link.status} · last active ${when(session.lastActiveAt)}`;
  });
}
