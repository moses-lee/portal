...
import { intentCheckPrompt, scopedSessions } from "./prompt.ts";
...
export function createIntents(core: JobsCore) {
  ...
  async function check({ job, run, trigger, signal }: KindContext): Promise<KindResult> {
    ...
    // The scoped sessions' live state goes into the prompt; a prefix id the list resolves is focused by its full id.
    const sessions = intent.scope.sessionIds.length ? await hub.deps.sessions.list().catch(() => null) : null;
    const sessionIds = intent.scope.sessionIds.map((id) => {
      const found = sessions ? scopedSessions(sessions, id) : [];
      return found.length === 1 ? found[0].id : id;
    });
    const prepared = await prepareTurn(hub, {
      ...
      interactive: false, toolNames: INTENT_CHECK_TOOLS, scope: { ...intent.scope, sessionIds }, query: `${intent.text}\n${intent.trigger}`, touched,
      summary: job.title,
    });
