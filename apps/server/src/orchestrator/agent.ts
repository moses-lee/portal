...
export function createOrchestratorAgent({ model, tools, system, providerOptions, maxSteps = MAX_STEPS }: {
  model: LanguageModel;
  tools: ToolSet;
  system: string;
  ...
  return new ToolLoopAgent({ model, tools, instructions: system, stopWhen: stepCountIs(Math.min(maxSteps, MAX_STEPS)), providerOptions });
}
