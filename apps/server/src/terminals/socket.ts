...
import { crossOriginError } from "../http/origin.ts";
import type { ShellEvent } from "../lib/shell-types.ts";
import { parseShellCommand } from "./commands.ts";
import type { createTerminalRegistry } from "./registry.ts";

const room = (terminalId: string) => `terminal:${terminalId}`;

/**
 * The terminal transport at `/api/shell/socket`. Browsers reach it through the Next.js rewrite, which
 * rewrites `Host` to this server's address and names the browser-facing host in `X-Forwarded-Host`,
 * so the handshake uses the same origin check as the HTTP routes instead of Socket.IO's own CORS.
 */
export function attachTerminalServer(server: HttpServer, terminals: ReturnType<typeof createTerminalRegistry>) {
  const io = new Server(server, {
    // No trailing slash: the Next.js proxy drops upgrade requests for `/api/shell/socket/`.
    path: "/api/shell/socket", addTrailingSlash: false, transports: ["websocket"], serveClient: false,
    maxHttpBufferSize: 128 * 1024,
    allowRequest(req, callback) {
      callback(null, crossOriginError(req) === null);
    },
  });
