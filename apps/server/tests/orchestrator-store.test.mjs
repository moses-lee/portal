...
import { orchestratorDocuments, orchestratorItems, orchestratorMessages } from "../src/db/schema.ts";
import {
  MAX_MEMORY_BYTES,
...
function behaviour(label, { open, reopen }) {
  ...
  test(`${label}: updateItem keeps only valid patch fields, so what is stored always loads`, async (t) => {
    const store = await open(t);
    const item = await store.createItem(itemInput());
...
test("postgres store: one row per record; memory and snapshot are documents", async (t) => {
...
test("parseItemPatch validates request bodies without a store", () => {
